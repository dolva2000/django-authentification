...
from appuser.user_utils import COUNTRIES_PHONE
...
class ConfirmResetCodeSZ(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        if user.reset_expired_at <= timezone.now():
            raise serializers.ValidationError(
                detail=f"Votre code a expiré. Veuillez faire une nouvelle demande de reinitialisation")
